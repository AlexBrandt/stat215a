...
result <- foreach(k = 2:k_max) %dopar% {
  ...
  for (i in 1:N){
    ...
    # Get the kmeans clustering labels with centers = k.
    L1 <- kmeans(lingBinary[sample1, 7:ncol(lingBinary)], centers = k)$cluster
    L2 <- kmeans(lingBinary[sample2, 7:ncol(lingBinary)], centers = k)$cluster
...
save(result, file = 'stability.Rdata')
